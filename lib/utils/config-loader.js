...
class ConfigLoader {
  constructor() {
    // No configDir needed - all configs in root
  }

  /**
   * Load GSuite emails from gsuite.txt
   * @returns {string[]} Array of email addresses
   */
  loadGSuiteConfig() {
    const filePath = path.join(process.cwd(), 'gsuite.txt');
    ...
    logger.info(`Loaded ${emails.length} GSuite emails from gsuite.txt`);
    return emails;
  }
   ...
   */
  loadGSuitePassword() {
    const filePath = path.join(process.cwd(), 'password-gsuite.txt');
   ...
   */
  loadGmailConfig() {
    const filePath = path.join(process.cwd(), 'gmail.txt');
    ...
    logger.info(`Loaded ${accounts.length} Gmail accounts from gmail.txt`);
    return accounts;
  }
   ...
   */
  loadProxyConfig() {
    const filePath = path.join(process.cwd(), 'proxy.txt');
   ...
   */
  load9RouterConfig() {
    const filePath = path.join(process.cwd(), '9router.json');
  ...
  validate() {
    ...
    const gsuiteEmails = this.loadGSuiteConfig();
    const gsuitePassword = this.loadGSuitePassword();
    const gmailAccounts = this.loadGmailConfig();
