...
class RouterClient {
  ...
  /**
   * Save refresh token to JSON file (9Router doesn't support manual import)
  ...
  async importToken(email, refreshToken) {
    try {
      ...
      const outputFile = path.join(process.cwd(), 'kiro-tokens.json');
      ...
      logger.success(`Token saved to ${outputFile}: ${email}`);
      return true;
    } catch (error) {
      logger.error(`Failed to save token for ${email}`, { error: error.message });
      return false;
    }
  }
}
